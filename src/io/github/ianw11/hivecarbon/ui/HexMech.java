...
*********************************************************************/
   public static void drawHex(int i, int j, Graphics2D g2) {
      ...
      // Fill inside
      g2.setColor(Hex.NO_PLAYER_COLOR);
      g2.fillPolygon(poly);
      
      // Draw border
      g2.setColor(GameBoard.COLOURGRID);
      g2.drawPolygon(poly);
   }
...
*Purpose: This draws a filled in polygon based on the coordinates of the hexagon.
     ...
     The colour is set by Driver.COLOURONE and Driver.COLOURTWO.
     The value of n is converted to letter and drawn in the hexagon.
*****************************************************************************/
   public static void fillHex(int i, int j, Hex currHex, Graphics2D g2) {
      int x = i * (s+t);
      int y = j * h + (i%2) * h/2;
      if (currHex.isActive()) {
         g2.setColor(currHex.getColor());
         g2.fillPolygon(hex(x,y));
         g2.setColor(GameBoard.COLOURONETXT);
         g2.drawString(currHex.getType().getShortName(), x+r+BORDERS, y+r+BORDERS+4); //FIXME handle XYVertex
      } else {
         g2.setColor(Color.GREEN);
         g2.fill(hex(x,y));
         g2.setColor(GameBoard.COLOURGRID);
         g2.drawString("Empty", x+r+BORDERS, y+r+BORDERS+4);
      }
